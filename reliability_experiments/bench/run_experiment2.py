...
from ensemble_boxes_wbf_experimental import weighted_boxes_fusion_experimental as wbf_baseline
from ensemble_boxes_wbf import weighted_boxes_fusion as wbf_per_box

# === NEW ===
from calibrate_reliabilities import get_or_estimate_reliabilities
...
def load_detector_preds_csv(csv_paths, column_map):
    """Reads detector predictions from multiple CSV files and returns per-image grouped boxes, scores, labels."""
...
def run(config):
    ...
    # === NEW: reliability calibration step ===
    pred_files = config.get("detector_preds", [])
    gt_path = config["coco"]["val_ann"]

    reliabilities = get_or_estimate_reliabilities(
        pred_files=pred_files,
        gt_path=gt_path,
        cache_path="outputs/reliabilities.json",
        metric="mAP",
        use_agreement=True
    )
    # =========================================
    for img_id, preds in image_predictions.items():
        if isinstance(img_id, str):
            img_id_clean = os.path.splitext(os.path.basename(img_id))[0]
            try:
                img_id_clean = int(img_id_clean)
            except ValueError:
                continue
        else:
            img_id_clean = int(img_id)
        cleaned_predictions[img_id_clean] = preds
    image_predictions = cleaned_predictions

    # --- Reliability fallback ---
    if not reliabilities or len(reliabilities) != len(config.get("detector_preds", [])):
        print("⚠️ No valid reliabilities computed; using uniform 1.0 weights")
        reliabilities = [1.0 for _ in config.get("detector_preds", [])]
    ...
    for method in config["methods"]:
        ...
        for image_id in tqdm(image_ids):
            ...
            boxes_list = [[
                [b[0], b[1], b[2], b[3]] for b in model_boxes
            ...
            if name == "WBF":
                ...
                fused_boxes, fused_scores, fused_labels = weighted_boxes_fusion_adaptive(
                    boxes_list,
                    scores_list,
                    labels_list,
                    weights=args.get("weights"),
